...
function VisaMark() {
  return (
    <span className="h-[20px] px-1.5 rounded border border-[#E7E5E0] bg-white flex items-center justify-center">
      <svg width="24" height="8" viewBox="0 0 40 13" aria-label="Visa">
...
function MastercardMark() {
  return (
    <span className="h-[20px] px-1.5 rounded border border-[#E7E5E0] bg-white flex items-center justify-center">
      <svg width="18" height="11" viewBox="0 0 24 16" aria-label="Mastercard">
...
function ApplePayMark() {
  return (
    <span className="h-[20px] px-1.5 rounded border border-[#E7E5E0] bg-white flex items-center justify-center">
      <svg width="14" height="11" viewBox="0 0 24 24" aria-label="Apple Pay">
...
function GooglePayMark() {
  return (
    <span className="h-[20px] px-1.5 rounded border border-[#E7E5E0] bg-white flex items-center justify-center">
      <svg width="28" height="11" viewBox="0 0 34 16" aria-label="Google Pay">
...
function PayPalMark() {
  return (
    <span className="h-[20px] px-1.5 rounded border border-[#E7E5E0] bg-white flex items-center justify-center">
      <svg width="42" height="10" viewBox="0 0 70 16" aria-label="PayPal">
...
function VenmoMark() {
  return (
    <span className="h-[20px] px-1.5 rounded border border-[#E7E5E0] bg-white flex items-center justify-center">
      <svg width="38" height="10" viewBox="0 0 60 16" aria-label="Venmo">
...
export function PaymentMethodCard({ variant, selected, onSelect, disabled, title, subtitle, icon, logos }: PaymentMethodCardProps) {
  ...
  return (
    <div
      ...
      className={`relative flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3 px-4 py-3 rounded-2xl bg-white cursor-pointer
        border-[1.5px] ${selected ? `border-2 ${v.borderSelected} ${v.bgSelected} ${v.glowSelected}` : `${v.borderDefault} shadow-[0_6px_14px_rgba(20,20,20,0.06)] hover:border-gray-400 hover:shadow-[0_8px_18px_rgba(20,20,20,0.10)]`}
    ...
    >
      ...
      <span
        className={`absolute -top-2 -right-2 w-5 h-5 rounded-full bg-[#3FA34D] text-white flex items-center justify-center text-xs font-bold shadow-[0_2px_6px_rgba(0,0,0,0.2)] transition-all duration-200 ${
      ...
      <div className="flex items-center gap-3">
        <div className={`shrink-0 w-11 h-11 rounded-xl flex items-center justify-center ${v.iconBg}`}>
          {icon}
        </div>

        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-gray-900 leading-snug">{title}</p>
          <p className="text-xs font-medium text-gray-500 leading-snug mt-0.5">{subtitle}</p>
        </div>

        {/* Desktop: logos + chevron sit to the right of the text */}
        <div className="hidden sm:flex items-center gap-2.5 shrink-0">
          <div className="flex items-center gap-1.5">{logos}</div>
          <span className="text-[#3A3A3A] text-base font-bold leading-none">›</span>
        </div>
      </div>

      {/* Mobile: logos wrap onto their own row below the text, indented under it */}
      <div className="flex sm:hidden items-center gap-1.5 flex-wrap pl-[56px]">
        {logos}
      </div>
    </div>
  );
